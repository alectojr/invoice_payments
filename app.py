...
# ====================== TRANSLATIONS ======================
TRANSLATIONS = { ... }  # Keep your existing TRANSLATIONS dict unchanged
...
class Payment(db.Model):
    # Keep your existing Payment model unchanged
    id = db.Column(db.Integer, primary_key=True)
...
class Receipt(db.Model):
    # Keep your existing Receipt model
    id = db.Column(db.Integer, primary_key=True)
...
# (generate_security_hash, send_invoice_sms, generate_invoice_no, etc.)
# Copy all of them from your original app.py here (I didn't repeat them to save space)
# ====================== HELPER FUNCTIONS ======================
...
def student_register():
    if request.method == 'POST':
        ...
        student = Student(
            ...
            course=course,
            address=address,
            program_type=request.form.get('program_type', 'Full Course'),
            user_id=user.id
        )
        db.session.add(student)
        db.session.commit()
        ...
        # Optional: Inform admin via flash that a new student registered
        return redirect(url_for('login'))
...
@app.route('/students/approve/<int:sid>')
def approve_student(sid):
    if session.get('user_type') not in ['admin', 'staff']: return redirect(url_for('students_list'))
    student = Student.query.get_or_404(sid)
    student.status = 'Approved'
    student.approved_by = session['user']
    student.approved_at = datetime.utcnow()
    db.session.commit()
...
def dashboard():
    ...
    if user_type in ['admin', 'staff']:
        students = Student.query.order_by(Student.enrollment_date.desc()).limit(10).all()
        ...
        students = payments = []
        total_students = pending = 0

    return render_template('dashboard.html', 
                         students=students, payments=payments,
                         total_students=total_students, pending=pending, search=search)

# ... (Keep all other original routes: login, new_invoice, etc.)
